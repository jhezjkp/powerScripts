...
def receiveData(clientfd):
    ...
    while True:
        ...
        if packet['md5']!=md5.hexdigest():
            logger.error("文件一致性校验失败: %s\t%s\t%s", path, packet['md5'], md5.hexdigest())
...
def loadConfig():
    ...
    '''
    xmldoc = minidom.parse(os.path.join(os.path.dirname(os.path.abspath(__file__)), "config_collectLogs.xml"))
    gameLogs = []
    nodeList = xmldoc.firstChild.getElementsByTagName("gameLogs")[0].getElementsByTagName("log")
    for node in nodeList:
        gameLogs.append(node.firstChild.data)
    loginLogs = []
    nodeList = xmldoc.firstChild.getElementsByTagName("loginLogs")[0].getElementsByTagName("log")
    for node in nodeList:
    ...
    for node in nodeList:
        ...
        server = Server(param["label"], param["name"], param["path"], param["name"].startswith("login") and loginLogs or gameLogs)
...
def gatherLogs(server, start=None, end=None):
    ...
    if not os.path.exists(logPath):
        logger.error("log path(%s) for %s doesn't exist!", server.path, server.label)
    else:
        patterns = []
        if start==None and end==None:
            ...
            for i in range(0, (end-start).days+1):
                patterns.append((start + datetime.timedelta(days=i)).strftime("%Y-%m-%d"))
        list = []
        count = 0
